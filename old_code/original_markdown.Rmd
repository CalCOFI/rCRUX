...
To use R Markdown, you need to install R Package Dependencies for R Markdown: 'rmarkdown', 'tinytex'


Goal of CRUX 2.0 is to create a CRUX library using R, a personal computer and possibly a flashdrive or external hard drive to hold big databases. The user needs to build taxonomizer taxonomy database, and download ncbi blast db and BLAST+.  Might be able to add in bold at some point.

Step 1. Use primerTree to primer BLAST NCBI for sequences that contain both primer pairs and retain only those within a specificed amplicon length (includes primer region).  - some day make a function for multiple primer sets / multiplexed etc.

Step 2. Using the primerTree output, loop through the dataframe one entry at a time. First, collect amplicon sequence from the first line in the table using blastdbcommand. Second, BLAST the amplicon and get up to 50,000 (max allowed) returns (+ the ocassional duplicated accession). Third, remove seqeunces found in the BLAST output table from the primerTree output and do it all again until no entries remain in the primerTree output.

In each iteration files are generated that store unique blast output (deduplicated by accession and amplicon length - only the longest seqeunce is retained.  Should add a check for multiple locations in an accession.  It totally happens.), duplicates found in individual BLAST attempts, hits with more than a given number of Ns (e.g. 4 in a row), and accessions that are in NCBI but not contained in the users BLAST database or taxonimizer database.

Step 4. Collect taxonomy using taxonomizer.  Currently it uses accessions to pull taxids and taxonomy from its sql database.  BLAST provides taxids, but some hits return multiple.  I checked a few instances of this phenomenon, and taxonomizer gave the same taxonomy found on NCBI nucleotide. 

#### work in progress ####

*  Take hits not found in the database and extract sequence (have script for that), append sequences to data table (do not have this working).

* Extract blast hits with no taxonomy and get them remotely.

* Extract data and format into fasta and taxonomy files

* Fix NCBI BLAST-db download function

* allow for searching of multiple blast databases

* Turn main chunks of scripts into functions

* Determine best way to store data - can R really handle keeping huge datasets open?

* parallelize blast - is this possible or even a good idea?

* Turn into R package

* lots of other notes in script for room for improvement...

* allow user to add their own sequence data

* make logs

* make more summary output

* check that functions run properly - main concern is that the variables are not being passed correctly or consistently....

* Check all "good" blast seeds make it into database

###########################
* small test.  Harbor porpus D loop.
...
other attached packages:
 [1] RCurl_1.98-1.3              data.table_1.14.0           taxonomizr_0.8.0            rlang_0.4.11               
 [5] rlist_0.4.6.1               ape_5.5                     forcats_0.5.1               stringr_1.4.0              
 [9] dplyr_1.0.7                 purrr_0.3.4                 readr_1.4.0                 tidyr_1.1.3                
[13] tibble_3.1.2                ggplot2_3.3.5               tidyverse_1.3.1             primerTree_1.0.5           
[17] gridExtra_2.3               directlabels_2021.1.13      ShortRead_1.50.0            GenomicAlignments_1.28.0   
[21] SummarizedExperiment_1.22.0 Biobase_2.52.0              MatrixGenerics_1.4.0        matrixStats_0.59.0         
[25] Rsamtools_2.8.0             GenomicRanges_1.44.0        Biostrings_2.60.1           GenomeInfoDb_1.28.1        
[29] XVector_0.32.0              IRanges_2.26.0              S4Vectors_0.30.0            BiocParallel_1.26.1        
[33] BiocGenerics_0.38.0         httr_1.4.2                  XML_3.99-0.6                lubridate_1.7.10           

loaded via a namespace (and not attached):
 [1] nlme_3.1-152           bitops_1.0-7           fs_1.5.0               RColorBrewer_1.1-2     tools_4.1.0           
 [6] backports_1.2.1        utf8_1.2.1             R6_2.5.0               DBI_1.1.1              colorspace_2.0-2      
[11] withr_2.4.2            tidyselect_1.1.1       compiler_4.1.0         cli_3.0.0              rvest_1.0.0           
[16] xml2_1.3.2             DelayedArray_0.18.0    scales_1.1.1           quadprog_1.5-8         digest_0.6.27         
[21] rmarkdown_2.9          jpeg_0.1-8.1           pkgconfig_2.0.3        htmltools_0.5.1.1      dbplyr_2.1.1          
[26] readxl_1.3.1           rstudioapi_0.13        generics_0.1.0         hwriter_1.3.2          jsonlite_1.7.2        
[31] magrittr_2.0.1         GenomeInfoDbData_1.2.6 Matrix_1.3-4           Rcpp_1.0.7             munsell_0.5.0         
[36] fansi_0.5.0            lifecycle_1.0.0        stringi_1.6.2          yaml_2.2.1             zlibbioc_1.38.0       
[41] plyr_1.8.6             grid_4.1.0             crayon_1.4.1           lattice_0.20-44        haven_2.4.1           
[46] hms_1.1.0              knitr_1.33             pillar_1.6.1           reshape2_1.4.4         codetools_0.2-18      
[51] reprex_2.0.0           glue_1.4.2             evaluate_0.14          latticeExtra_0.6-29    modelr_0.1.8          
[56] png_0.1-7              vctrs_0.3.8            foreach_1.5.1          cellranger_1.1.0       gtable_0.3.0          
[61] assertthat_0.2.1       xfun_0.24              broom_0.7.8            iterators_1.0.13       ellipsis_0.3.2
...
Functions:
```{r}
# require packages - Zack!
# make package download - Emily
# 
...
###############################
# Set up taxonomizr accessionTaxa.sql taxonomy database 
# Downloads the Accession2taxid files, and set up the taxonimizr database.  
# I set this up on an external harddrive.  It will be at least 62 GB


set_up_taxonomizr <- "N" #"Y" for yes
taxonomizr_directory <- "/Volumes/20170320_eDNA_seq_data/Blast_and_taxo/accession2taxid/"
#
#
if (set_up_taxonomizr == "Y") {
  dir.create(file.path(taxonomizr_directory))
  print("this may take a while")
  prepareDatabase(paste0(taxonomizr_directory,'accessionTaxa.sql',sep = ''))
}
#
#############################
...
#############################
# Download BLAST+
# Indicate file path for download
#
blast_tools_dir <- "/Volumes/20170320_eDNA_seq_data/Blast_and_taxo/Blast_tools/"
download_blast_exe("blast_tools_dir")
#
############################
...
# Indicate file path for download
#
#blast_db_dir <- "/Volumes/20170320_eDNA_seq_data/Blast_and_taxo/Blast_nt_db3/"
#download_blast_db(blast_db_dir)
#
############################
...
Single Taxid - run these:

Step 1. Get BLAST seeds  ---.
# add summary statistics... 

```{r}
# make each primer blast return append not overwrite - Emily

##################
# Run get_blast_seeds: it uses primerTree to search for primers and return - at maximum   the specified number of alignments.  
# It is limited to 50,000 returns.
# 
# Here is the usage:
# get_blast_seeds(forward_primer, reverse_primer, file_out_dir, Metabarcode_name, accessionTaxa, num_aligns = 50000, organism =  organism_to_search, num_permutations = 25, primer_specificity_database = "nt", mismatch = 3, minimum_length = 5, maximum_length = 500)

get_blast_seeds("TAGAACAGGCTCCTCTAG" , "TTAGATACCCCACTATGC" , "/Users/limeybean/Dropbox/CRUX_2.0/", "12S_V5F1","/Volumes/20170320_eDNA_seq_data/Blast_and_taxo/accession2taxid/accessionTaxa.sql", primer_specificity_database = "nt", num_permutations = 50, organism = c("7776"), mismatch = 5, minimum_length = 5, maximum_length = 1000)  

## means try different - more specifc - level of taxonomy
...
#get_blast_seeds("GCHCCHGAYATRGCHTTYCC" , "ARYATDGTRATDGCHCCDGC", "/Users/limeybean/Dropbox/CRUX_2.0/", "CO1_Elbrecht_7735","/Volumes/20170320_eDNA_seq_data/Blast_and_taxo/accession2taxid/accessionTaxa.sql", primer_specificity_database = "nt", num_permutations = 50, organism = c("7735"), mismatch = 5, minimum_length = 5, maximum_length = 1000) 

#"10226","1476529","28009", "2687318", "4751", "127916", "2686024", "2687318","2316435","6040", "6073", "7712", "7735", "7776"

```


Step 2. Restart blast from run failure.....
fun-its starts on blast 163
```{r}

# how to make blast go faster - more reads at a time - multi threading?  - blast replacement? -- JESSE!!!!!!
# metablaster - Emily Jesse

#########
# Run rcrux_bast: it is a big serial blast loop that runs until all entries from the primerTree output have been blasted.  It also picks up blasting from a crapped out run. It keeps track of all blast returns, entries rejected due to to many Ns or found to be missing in the users blast database.
# Needs user modifiable options for blast parameters....
# usage:
# rcrux_blast <- function(file_out_dir, Metabarcode_name, blast_tools, blast_db, accessionTaxa, number_Ns_in_blast_seed = "NNNN")


# have to add a column at the end of the file called "database_used_for_blast"  all entries should be "nt"
rcrux_blast("/Users/limeybean/Dropbox/CRUX_2.0/12S_V5F/","12S_V5F", "/Users/limeybean/Downloads/ncbi-blast-2.10.1+", "/Volumes/20170320_eDNA_seq_data/Blast_and_taxo/Blast_nt_db/nt","/Volumes/20170320_eDNA_seq_data/Blast_and_taxo/accession2taxid/accessionTaxa.sql")



#check that all blast inputs not in the too-new-for-you file are included in blasted file
```


For multiple Taxid at a time:



```{r}

#hypothetically.. should work with multiple taxid on nt and ref_seq databases.  In practice it works best with just nt and non degenerate primers.

get_blast_seeds_multi_taxa_or_db ("ATGCGATACTTGGTGTGAAT" , "GACGCTTCTCCAGACTACAAT", "/Users/limeybean/Dropbox/CRUX_2.0/", "PITS-3","/Volumes/20170320_eDNA_seq_data/Blast_and_taxo/accession2taxid/accessionTaxa.sql", num_permutations = 50, db_to_blast = c("nt"), organism = c("35493", "3041"), mismatch = 5, minimum_length = 350, maximum_length = 550) 


#get_blast_seeds_multi_taxa_or_db("GCHCCHGAYATRGCHTTYCC" , "ARYATDGTRATDGCHCCDGC" , "/Users/limeybean/Dropbox/CRUX_2.0/", "CO1_BF2_Elbrecht","/Volumes/20170320_eDNA_seq_data/Blast_and_taxo/accession2taxid/accessionTaxa.sql", num_permutations = 1, db_to_blast = c("nt"), organism = c( "10226","1476529","28009", "2687318", "4751", "127916", "2686024", "2687318","2316435","6040", "6073", "7712", "7735", "776"), mismatch = 7, minimum_length = 15, maximum_length = 700)  


# list of taxid for major metazoan lineages....
#"10226","1476529","28009", "2687318", "4751", "127916", "2686024", "2687318","2316435","6040", "6073", "7712", "7735", "776"


rcrux_blast("/Users/limeybean/Dropbox/CRUX_2.0/PITS-3/","PITS-3", "/Users/limeybean/Downloads/ncbi-blast-2.10.1+", "/Volumes/20170320_eDNA_seq_data/Blast_and_taxo/Blast_db/" ,"/Volumes/20170320_eDNA_seq_data/Blast_and_taxo/accession2taxid/accessionTaxa.sql")
...
For future work...........
Step 3. **Using taxonomizr** fetch taxonomy associated with the Blast results - if blast is newer than the taxonimizer file - could have accessions without taxonomy...

```{r}
# test function - emily

####################################
# run fetch_rcrux_taxonomy: should work on any file with accession numbers....
#
# usage:
# fetch_rcrux_taxonomy(blast_out, accessionTaxa, file_out_dir, Metabracode_name)

blast_out = read_csv("/Users/limeybean/Dropbox/CRUX_2.0/12S-100/12S-100_blasted_entries.csv")

fetch_rcrux_taxonomy(blast_out, "/Volumes/20170320_eDNA_seq_data/Blast_and_taxo/accession2taxid/accessionTaxa.sql", "/Users/limeybean/Dropbox/CRUX_2.0/12S-100/12S-100", "12S-100")


```

Step 4. Convert hits to fasta format and taxonomy
```{r}

# make function - find function - ZACK!!!!!

#### work in progress....

get_fasta_for_seq <- function(csv){    
    seq = csv$sequence
    names(seq) = csv$accession
    dna = DNAStringSet(seq)
    write.XStringSet(dna, "low_id.fasta")
}
...
fasta[c(TRUE, FALSE)] <- paste0(">", dupt_no_hiyp$accession)
fasta[c(FALSE, TRUE)] <- dupt_no_hiyp$sequence
writeLines(fasta, "/Users/limeybean/Dropbox/CRUX_2.0/Fun-ITS_test/filename.fasta")

```


Code for fetching reads directly from NCBI

```{r}
# obtain ncbi sequence for ncbi entries not included in users blast database.  Update blast regularly to avoid huge numbers of reads...
# turn into function
if (too_new_for_you$product_start < too_new_for_you$product_stop) {
  amplicon_start <- too_new_for_you$forward_stop + 1
  amplicon_stop <- too_new_for_you$product_stop - 1
} else {
  amplicon_start <- too_new_for_you$product_start - 1
  amplicon_stop <- too_new_for_you$product_stop + 1
}

too_new_for_you <- too_new_for_you %>% slice(-2)
too_new_for_you_with_seq <- get_sequences(too_new_for_you$accession, start=too_new_for_you$forward_stop, stop=too_new_for_you$reverse_stop, api_key = API_key)
too_new_for_you_with_seq1 <- too_new_for_you_with_seq

too_new_for_you_with_seq1 <- as.character(too_new_for_you_with_seq)

too_new_for_you_with_seq2 <- do.call(rbind,too_new_for_you_with_seq1)

#Find way to append sequences to table...
#library(magrittr)
#library(Biostrings)

#too_new_for_you_with_seq2 <- too_new_for_you_with_seq %>% as.character %>% lapply(.,paste0,collapse="") %>% unlist %>% DNAStringSet

#too_new_for_you1 <- full_join(too_new_for_you, too_new_for_you_with_seq, by = "accession")
