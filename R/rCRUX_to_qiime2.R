...
#' \dontrun{
#'  
#'  convert_to_qiime2(ref_db_1_path=here("eecurd_updated_dbs_20230420","expanded_12S_20230420","12S","blast_seeds_output","12S_taxonomy.txt"),ref_db_1_name="12S",out_dir=here())
#' }
#'
#' @export
#'  

convert_to_qiime2 <- function(ref_db_1_path,ref_db_1_name,out_dir, format_ref_db_1=FALSE) {
  ...
  ref_db_1_qiime <- ref_db_1 %>%  dplyr::mutate(., species = str_replace_all(species," ","_"),
    ...
    tidyr::unite(sum.taxonomy, c("superkingdom","phylum","class","order","family","genus","species"), sep=";") 
  
  taxa_table_path <- file.path(out_dir, paste0(ref_db_1_name, "_qiime2_taxonomy.txt"))
  utils::write.table(ref_db_1_qiime, file = taxa_table_path, row.names = FALSE, col.names=FALSE, sep = "\t", quote = FALSE)
